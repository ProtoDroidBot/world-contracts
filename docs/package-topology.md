...
Faction policy is split into files rather than duplicating deployment metadata. `factions/default.v1.json` uses format `eve-frontier-faction-features`, policy schema 2, `configId: "default"`, and a capability allowlist. Every canonical `factionID-factionStringOnlyID` entry has its own `factions/<faction-key>.v1.json` and explicitly references fallback `default`. A faction file may omit `capabilities` to inherit the default or provide an array to replace the default allowlist. Policy schema 2 also records the faction transponder code, dynamic `npcProfiles` type-list selectors and type-ID overrides, expanded ally/enemy contacts, one combined `leadership` list, and a separate `commanders` list. Contacts are bound to canonical faction keys and the target's current code. Character lists accept NPC or player IDs but are empty in the shipped configuration so later authority assignment remains dynamic. The manifest and faction files are synchronized and validated together; traversal, aliases, reparse points, mismatched faction identities, stale contact codes, unknown capabilities, and references to undeployed capabilities are rejected. The explicit file/reference boundary is intended to map cleanly to later on-chain faction objects. A later on-chain migration must publish only the existing transponder commitment, never the plaintext code or salt.

Faction policy also carries `startingRegion: { regionID, solarSystemIDs }`. The region ID takes precedence: future NPC respawn selection can choose a random known system in that region. The explicit system list is used only when the region cannot be resolved, including as an SDE home-system fallback. This is off-chain seed policy, not a Move contract or a replacement for persisted NPC location and migration state.

In the complete EveJS workspace, a fresh deployment automatically uses the root `npc-factions.config.json` schema-2 unified source and creates the default plus one policy file for every configured faction before atomically publishing the root manifest. `WORLD_FACTION_FEATURES_SOURCE` can select another source shaped like `config/faction-features.example.json`; a standalone world-contracts checkout with neither source simply omits faction policy. Canonical keys are derived from the explicit numeric/string identity fields, never from display names or transponder signals.
