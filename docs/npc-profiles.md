...
The EveJS server reads `EVEJS_SUI_NPC_PACKAGE_ID`, `EVEJS_SUI_NPC_TYPE_ORIGIN`, and `EVEJS_SUI_NPC_REGISTRY_ID`, or the `npc` capability in public `world-features.v1.json` beside its synchronized `world.private.json`. `EVEJS_SUI_WORLD_FEATURES_CONFIG_PATH` selects an explicit versioned manifest; `EVEJS_SUI_NPC_CONFIG_PATH` remains a legacy alias. Environment overrides take precedence per field but cannot mask malformed or mismatched manifest data. The server revalidates the deployment fingerprint before submitting a signed operation.

The versioned manifest represents NPC, assembly-access, catapult, Smart Industry, transponder, queue, Industry Actions, Logistics Actions, Infrastructure Actions, and Automation independently. An omitted/unavailable record does not invalidate the others. Historical flat schemas 1–3 remain migration inputs. The complete schema and upgrade invariants are documented in [Package topology and deployment identity](package-topology.md). In particular, assembly access and the canonical action queue have independent package, type-origin, and registry identities; they must never be inferred from the NPC package in a split deployment.

Keep the authoritative manifest in `deployments/localnet/world-features.v1.json`. EveJS `FrontierWorld.ps1 sync` validates and copies its public runtime records beside `world.private.json`, retaining the original world identity. Faction policy is split into `factions/default.v1.json` and one canonical `factions/<factionID-factionStringOnlyID>.v1.json` per configured faction; every faction reference names fallback `default`. Global package addresses are not duplicated per faction. Synchronization requires canonical full-length nonzero addresses and rejects malformed or mismatched manifests and faction files. A historical `npc-deployment.json` is migrated, incomplete triples are reported but not promoted, and a historical synchronized destination is archived. If the source is absent but a destination manifest exists, sync fails closed and preserves that file for explicit reconciliation. `sync -DryRun` performs validation without changing files. Synchronization itself does not publish or upgrade contracts.
